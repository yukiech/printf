...
/*   Updated: 2021/10/30 07:52:12 by ahuber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_var(const char *to_print, va_list args, int i, int *count)
{
	if (to_print[i + 1] == 's')
		...
		ft_putstr_p(ft_itoa_base_x(va_arg(args, int), 16), count);
	else if (to_print[i + 1] == 'X')
		ft_putstr_p(ft_str_toup(ft_itoa_base_x(va_arg(args, int), 16)), count);
